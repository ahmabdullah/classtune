...
import com.classtune.classtuneapp.schoolapp.fragments.SchoolFeedFragment;
...
public class PaidVersionHomeFragment extends Fragment implements
	...
	private void setUp() {
		...
		for (int i = 0; i < myDiaryArrayText.length; i++) {
			...
			child.setClazzName("com.classtune.classtuneapp.schoolapp.fragments."
					+ myDiaryArrayClazz[i]);
	...
	public void onClick(View v) {
		...
		if(!ispaid && Integer.parseInt(child.getId()) > 2){
			showCustomDialog(child.getText().toUpperCase(),R.drawable.premium_upgrade_icon, "Contact your school to Upgrade.");
			return;
		}
	...
	private void loadDiaryItem(DrawerChildMenuDiary item) {
		...
		try {
			...
			if (className
					.equals("com.classtune.classtuneapp.schoolapp.fragments.SchoolFeedFragment")) {
